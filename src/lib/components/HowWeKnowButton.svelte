...
<div class="w-[400px] absolute bottom-4 left-32 transition-all " bind:this={infoBox}>
    <button class="absolute z-10 transition-all duration-500 left-5 {isHovered ? "boop brightness-125":""}" style="top: {isOpen ? "-32":reservedTextHeight-64}px" on:click={()=>isOpen=!isOpen} on:mouseover={()=>isHovered=true} on:focus={()=>isHovered=true} on:mouseout={()=>isHovered=false} on:blur={()=>isHovered=false}>
    ...
    <button class="absolute transition-all bottom-7 left-20 {color==="red" ? "text-light-pink" : "text-light-"+color} pointer-events-auto duration-500 {isHovered ? "brightness-125":"" } {isOpen ? "-bottom-[100%] opacity-0":"bottom-4"}" on:click={()=>isOpen=!isOpen} on:mouseover={()=>isHovered=true} on:focus={()=>isHovered=true} on:mouseout={()=>isHovered=false} on:blur={()=>isHovered=false} >HOW WE KNOW</button>
    ...
    <div class="w-full rounded-lg 
        ...
        <a class="bump flex flex-row pt-5 gap-5  hover:brightness-125 {color==="red" ? "text-light-pink" : "text-light-"+color}" href={reportLink} target="_blank">
